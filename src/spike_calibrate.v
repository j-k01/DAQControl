...
//
//   cal[15:0]  = gain, SIGNED Q2.14 (0x4000 = +1.000x, 0xC000 = -1.000x,
//                range ~+/-2x) -- a negative gain INVERTS that neuron's
//                pulse. 0 selects unity so an unprogrammed register passes
...
module spike_calibrate (
    ...
    wire signed [15:0] gain_q2_14 = (cal[15:0] == 16'd0) ? 16'sh4000
                                                         : $signed(cal[15:0]);
    wire signed [15:0] offset_s16 = cal[31:16];

    // stage 1: per-lane DSP multiply, s16 x signed Q2.14
    reg signed [31:0] prod0 = 32'd0, prod1 = 32'd0,
                      prod2 = 32'd0, prod3 = 32'd0;
    reg signed [15:0] off_p = 16'd0;

    // stage 2: Q2.14 rescale + offset + symmetric s16 saturation
    function [15:0] sat_cal;
        input signed [31:0] prod;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            prod0 <= $signed(in_word[15:0])  * gain_q2_14;
            prod1 <= $signed(in_word[31:16]) * gain_q2_14;
            prod2 <= $signed(in_word[47:32]) * gain_q2_14;
            prod3 <= $signed(in_word[63:48]) * gain_q2_14;
            off_p <= offset_s16;
