...
module adc_axis_capture_streamer #(
    ...
    localparam [COUNT_W-1:0] MAX_FRAMES = CAPTURE_FRAMES;

    reg                 running = 1'b0;
    reg                 done = 1'b0;
    reg [COUNT_W-1:0]   active_frames = MAX_FRAMES;
    reg [COUNT_W-1:0]   accepted_count = {COUNT_W{1'b0}};
    reg [15:0]          stalled_count = 16'd0;

    wire [COUNT_W-1:0] requested_frames =
        ((frame_count == 32'd0) || (frame_count > CAPTURE_FRAMES)) ?
            MAX_FRAMES :
            frame_count[COUNT_W-1:0];
    wire output_fire = m_axis_tvalid & m_axis_tready;
    wire load_frame = running & data_valid & (~m_axis_tvalid | output_fire);
    wire next_is_last = accepted_count == (active_frames - {{(COUNT_W-1){1'b0}}, 1'b1});
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            active_frames <= requested_frames;
            accepted_count <= {COUNT_W{1'b0}};
            stalled_count <= 16'd0;
            m_axis_tdata <= 128'd0;
            m_axis_tlast <= 1'b0;
